# Generated by Django 3.2.12 on 2022-02-10 09:24

import datetime
from django.db import migrations, models
import django.db.models.deletion
import multiselectfield.db.fields
import uuid


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        ('contenttypes', '0002_remove_content_type_name'),
    ]

    operations = [
        migrations.CreateModel(
            name='EndPage',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=100)),
                ('title', models.CharField(max_length=255)),
                ('javascripts', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/js/classical_information.js', 'experiment/js/classical_information.js'), ('experiment/js/classical_main.js', 'experiment/js/classical_main.js')], max_length=70, null=True)),
                ('styles', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/css/classical_end.css', 'experiment/css/classical_end.css'), ('experiment/css/classical_main.css', 'experiment/css/classical_main.css'), ('experiment/css/hide_footer.css', 'experiment/css/hide_footer.css'), ('experiment/css/hide_sidebar.css', 'experiment/css/hide_sidebar.css')], max_length=129, null=True)),
                ('content', models.JSONField(blank=True, null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('template', models.CharField(choices=[('pages/end/classical_end.html', 'pages/end/classical_end.html')], help_text='You can add templates into: pages/end/classical_end.html', max_length=255)),
            ],
            options={
                'verbose_name': '[Page] end',
                'verbose_name_plural': '[Page] ends',
            },
        ),
        migrations.CreateModel(
            name='ExamplePage',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=100)),
                ('title', models.CharField(max_length=255)),
                ('javascripts', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/js/classical_information.js', 'experiment/js/classical_information.js'), ('experiment/js/classical_main.js', 'experiment/js/classical_main.js')], max_length=70, null=True)),
                ('styles', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/css/classical_end.css', 'experiment/css/classical_end.css'), ('experiment/css/classical_main.css', 'experiment/css/classical_main.css'), ('experiment/css/hide_footer.css', 'experiment/css/hide_footer.css'), ('experiment/css/hide_sidebar.css', 'experiment/css/hide_sidebar.css')], max_length=129, null=True)),
                ('content', models.JSONField(blank=True, null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('template', models.CharField(choices=[('pages/examples/classical_example.html', 'pages/examples/classical_example.html')], help_text='You can add templates into: pages/examples/classical_example.html', max_length=255)),
            ],
            options={
                'verbose_name': '[Page] example',
                'verbose_name_plural': '[Page] examples',
            },
        ),
        migrations.CreateModel(
            name='Experiment',
            fields=[
                ('id', models.AutoField(editable=False, primary_key=True, serialize=False, unique=True)),
                ('title', models.CharField(max_length=255)),
                ('slug', models.SlugField(blank=True, editable=False, help_text='This field is not required and will be generated automatically when the object is saved based on the title of the experiment', max_length=255, unique=True)),
                ('description', models.TextField()),
                ('is_available', models.IntegerField(blank=True, choices=[(1, 'Available'), (0, 'Disabled')], default=1, null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('config', models.JSONField(blank=True, null=True)),
                ('end_page', models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='experiment', to='experiments.endpage')),
                ('example_page', models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='experiment', to='experiments.examplepage')),
            ],
            options={
                'verbose_name': '[Experiment] experiment',
                'verbose_name_plural': '[Experiment] experiments',
                'ordering': ['title'],
            },
        ),
        migrations.CreateModel(
            name='InformationPage',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=100)),
                ('title', models.CharField(max_length=255)),
                ('javascripts', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/js/classical_information.js', 'experiment/js/classical_information.js'), ('experiment/js/classical_main.js', 'experiment/js/classical_main.js')], max_length=70, null=True)),
                ('styles', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/css/classical_end.css', 'experiment/css/classical_end.css'), ('experiment/css/classical_main.css', 'experiment/css/classical_main.css'), ('experiment/css/hide_footer.css', 'experiment/css/hide_footer.css'), ('experiment/css/hide_sidebar.css', 'experiment/css/hide_sidebar.css')], max_length=129, null=True)),
                ('content', models.JSONField(blank=True, null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('template', models.CharField(choices=[('pages/information/classical_information.html', 'pages/information/classical_information.html')], help_text='You can add templates into: pages/information/classical_information.html', max_length=255)),
            ],
            options={
                'verbose_name': '[Page] information',
                'verbose_name_plural': '[Page] informations',
            },
        ),
        migrations.CreateModel(
            name='MainPage',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=100)),
                ('title', models.CharField(max_length=255)),
                ('javascripts', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/js/classical_information.js', 'experiment/js/classical_information.js'), ('experiment/js/classical_main.js', 'experiment/js/classical_main.js')], max_length=70, null=True)),
                ('styles', multiselectfield.db.fields.MultiSelectField(blank=True, choices=[('experiment/css/classical_end.css', 'experiment/css/classical_end.css'), ('experiment/css/classical_main.css', 'experiment/css/classical_main.css'), ('experiment/css/hide_footer.css', 'experiment/css/hide_footer.css'), ('experiment/css/hide_sidebar.css', 'experiment/css/hide_sidebar.css')], max_length=129, null=True)),
                ('content', models.JSONField(blank=True, null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('template', models.CharField(choices=[('pages/main/classical_main.html', 'pages/main/classical_main.html')], help_text='You can add templates into: pages/main/classical_main.html', max_length=255)),
            ],
            options={
                'verbose_name': '[Page] main',
                'verbose_name_plural': '[Page] mains',
            },
        ),
        migrations.CreateModel(
            name='Participant',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=100)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
            ],
            options={
                'verbose_name': '[Experiment] participant',
                'verbose_name_plural': '[Experiment] participants',
                'ordering': ['created_on'],
            },
        ),
        migrations.CreateModel(
            name='Session',
            fields=[
                ('id', models.AutoField(editable=False, primary_key=True, serialize=False, unique=True)),
                ('name', models.CharField(max_length=255)),
                ('estimated_duration', models.DurationField(default=datetime.timedelta(0), help_text='hh:mm:ss')),
                ('progress_choice', models.CharField(choices=[('default', 'default')], help_text='You can add progress classes into: expe/experiments', max_length=255)),
                ('is_active', models.IntegerField(blank=True, choices=[(1, 'Active'), (0, 'Inactive')], default=1, help_text='Session will be displayed but not accessible', null=True)),
                ('is_available', models.IntegerField(blank=True, choices=[(1, 'Available'), (0, 'Disabled')], default=1, help_text='Session will not be displayed if disabled', null=True)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('config', models.JSONField(blank=True, null=True)),
                ('experiment', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='sessions', to='experiments.experiment')),
            ],
            options={
                'verbose_name': '[Experiment] session',
                'verbose_name_plural': '[Experiment] sessions',
            },
        ),
        migrations.CreateModel(
            name='SessionProgress',
            fields=[
                ('id', models.AutoField(editable=False, primary_key=True, serialize=False, unique=True)),
                ('is_started', models.BooleanField(default=False, editable=False)),
                ('is_finished', models.BooleanField(default=False, editable=False)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('data', models.JSONField(blank=True, null=True)),
                ('participant', models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='progresses', to='experiments.participant')),
                ('polymorphic_ctype', models.ForeignKey(editable=False, null=True, on_delete=django.db.models.deletion.CASCADE, related_name='polymorphic_experiments.sessionprogress_set+', to='contenttypes.contenttype')),
                ('session', models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='progresses', to='experiments.session')),
            ],
            options={
                'abstract': False,
                'base_manager_name': 'objects',
            },
        ),
        migrations.CreateModel(
            name='ClassicalSessionProgress',
            fields=[
                ('sessionprogress_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='experiments.sessionprogress')),
            ],
            options={
                'abstract': False,
                'base_manager_name': 'objects',
            },
            bases=('experiments.sessionprogress',),
        ),
        migrations.CreateModel(
            name='SessionStep',
            fields=[
                ('id', models.UUIDField(default=uuid.uuid4, editable=False, primary_key=True, serialize=False)),
                ('created_on', models.DateTimeField(auto_now_add=True)),
                ('progress_id', models.PositiveIntegerField()),
                ('binary', models.BinaryField(blank=True, null=True)),
                ('data', models.JSONField(blank=True, null=True)),
                ('progress_type', models.ForeignKey(on_delete=django.db.models.deletion.PROTECT, related_name='steps', to='contenttypes.contenttype')),
            ],
            options={
                'verbose_name': '[Experiment] session step',
                'verbose_name_plural': '[Experiment] session steps',
            },
        ),
        migrations.AddField(
            model_name='experiment',
            name='information_page',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='experiment', to='experiments.informationpage'),
        ),
        migrations.AddField(
            model_name='experiment',
            name='main_page',
            field=models.ForeignKey(null=True, on_delete=django.db.models.deletion.PROTECT, related_name='experiment', to='experiments.mainpage'),
        ),
        migrations.CreateModel(
            name='ClassicalSessionProgressTime',
            fields=[
                ('classicalsessionprogress_ptr', models.OneToOneField(auto_created=True, on_delete=django.db.models.deletion.CASCADE, parent_link=True, primary_key=True, serialize=False, to='experiments.classicalsessionprogress')),
            ],
            options={
                'abstract': False,
                'base_manager_name': 'objects',
            },
            bases=('experiments.classicalsessionprogress',),
        ),
    ]
